...
static void read_ps2(void) {
    ...
    while (1) {
        ...
        if (ext) {
            ...
            /* [FIX-B2] Always clear ext here. If a spurious ext prefix
             * arrived before a regular key (e.g. Enter/ESC), the old code
             * left ext=true, causing the NEXT regular byte to be mis-routed
             * through the extended switch and silently dropped. */
            ext = false;
            switch (byte) {
...
static void vga_init(void) {
    /* [FIX-B3] Tell the pixel buffer controller which buffer to display.
     * Without this write CPUlator shows garbage or a blank screen because
     * the controller's "front buffer" register (pbc[1]) is uninitialised. */
    pbc[1] = PIX_BUF1;
...
/* No buffer swap — single buffer mode for CPUlator.
 * [FIX-B1] The old code polled pbc[3] & 1u which NEVER clears without a
 * real swap request, causing an infinite loop and freezing the display.
 * With a single buffer we simply return immediately; the timer_wait() call
 * in main() already provides ~60 Hz pacing via the interval timer. */
static void swap_buffers(void) {
    /* Nothing to do — single-buffer, no swap needed. */
}
